...
public class PartialSpecLocation implements Spec<Partial, PartialSpecLocation> {
	...
	public static class Serializer implements JsonSerializer<PartialSpecLocation> {
		...
		public void write(PartialSpecLocation thing, JsonObject json) {
			json.add("predicate", LocationPredicate.CODEC.encode(thing.pred, com.mojang.serialization.JsonOps.INSTANCE, null).getOrThrow(true, (i) -> {}));
		...
		public PartialSpecLocation read(JsonObject json) {
			LocationPredicate pred = LocationPredicate.CODEC.decode(com.mojang.serialization.JsonOps.INSTANCE, json.get("predicate")).getOrThrow(true, (i) -> {}).getFirst();
